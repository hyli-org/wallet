...
export const getErrorMessage = (error: unknown): ErrorDetails => {
    ...
    if (errorString.includes("Transaction failed") || errorString.includes("Transaction timed out")) {
        return {
            userMessage: "Your transaction couldn't be completed. This might be due to network issues or insufficient funds.",
            technicalMessage: errorString,
            showDetails: true
        };
    }
    ...
    if (errorString.includes("wallet does not exist")) {
        return {
            userMessage: "No account found with this username. Please check your username or create a new account.",
            technicalMessage: errorString
        };
    }

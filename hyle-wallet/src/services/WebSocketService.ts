...
export class WebSocketService {
  ...
  connect(account: string) {
    if (this.ws) {
      ...
      if (this.currentAccount === account) {
        ...
        if (this.ws.readyState === WebSocket.OPEN) {
          ...
          console.log("WebSocket for the same account exists but is not open. Cleaning up old socket and preparing for new connection.");
          this.ws.onopen = null;
    ...
    this.ws.onopen = () => {
      ...
      if (this.ws && this.currentAccount) { // Ensure ws and currentAccount are still valid post-async
    ...
    this.ws.onclose = () => {
      console.log(`WebSocket disconnected. Current account: ${this.currentAccount}. Reconnect attempts: ${this.reconnectAttempts}`);
  ...
  private handleReconnect() {
    if (this.currentAccount) { // Only proceed if there's an account context for reconnection
      if (this.reconnectAttempts < this.maxReconnectAttempts) {
        ...
        console.log(`Max reconnect attempts (${this.maxReconnectAttempts}) reached for account ${this.currentAccount}. Will not reconnect further. Call connect() manually to re-initiate.`);
        ...
        console.log("Reconnect attempt skipped: no current account set (likely disconnected intentionally or already handled).");
    }
  }
